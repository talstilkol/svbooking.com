...
| `npm test` | PASS | 161 test files, 795 tests passed. |
| `npm run test:coverage` | PASS | Coverage command runs with `@vitest/coverage-v8`; current `lib` coverage is 90.67% lines, 86.33% statements, 90.04% functions, and 76.07% branches. |
...
| Branch coverage below next target | Medium | `lib` branch coverage is 76.07%. | Add focused tests for cache/provider/auth/error branches, then raise the coverage ratchet toward 80%. |
