...
| `npm run test:coverage` | PASS | Coverage command runs with `@vitest/coverage-v8`; current `lib` coverage is 99.44% lines, 99.09% statements, 97.39% functions, and 99.72% branches. |
| `npm run audit:coverage` | PASS | Coverage ratchet prevents regression below the current floors: lines 99.4%, statements 99.0%, functions 97.3%, branches 99.7%. |
...
- Raised provider observability, catalog media, URL-safety, reviews, i18n, REST Countries, DBpedia, candidate-review, webhook URL, Wikivoyage, Ticketmaster, cheaper-date, Xotelo, auth helper, cron-auth helper, and price-cache edge coverage, then moved the branch coverage ratchet floor to 99.7%.
...
| Branch coverage next target | Medium | `lib` branch coverage is 99.72%. | Continue focused tests for remaining private/inaccessible hotels-catalog, hotel-popularity, ops-alerts, webhook URL, and price-cache branches, then raise the next ratchet beyond 99.7%. |
