...
| `npm run test:coverage` | PASS | Coverage command runs with `@vitest/coverage-v8`; current `lib` coverage is 99.44% lines, 98.99% statements, 97.39% functions, and 99.50% branches. |
| `npm run audit:coverage` | PASS | Coverage ratchet prevents regression below the current floors: lines 99.4%, statements 98.9%, functions 97.3%, branches 99.5%. |
...
- Raised provider observability, catalog media, URL-safety, reviews, i18n, REST Countries, DBpedia, candidate-review, webhook URL, and price-cache edge coverage, then moved the branch coverage ratchet floor to 99.5%.
...
| Branch coverage next target | Medium | `lib` branch coverage is 99.50%. | Continue focused tests for remaining private/inaccessible hotels-catalog, auth, hotel-popularity, Xotelo, ops-alerts, cheaper-dates, Wikivoyage, Ticketmaster, and price-cache branches, then raise the next ratchet beyond 99.5%. |
