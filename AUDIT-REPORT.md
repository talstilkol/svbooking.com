...
| `npm test` | PASS | 165 test files, 854 tests passed. |
| `npm run test:coverage` | PASS | Coverage command runs with `@vitest/coverage-v8`; current `lib` coverage is 93.25% lines, 89.87% statements, 92.66% functions, and 82.25% branches. |
| `npm run audit:coverage` | PASS | Coverage ratchet prevents regression below the current floors: lines 93.2%, statements 89.8%, functions 92.6%, branches 82.2%. |
...
| Branch coverage next target | Medium | `lib` branch coverage is 82.25%. | Add focused tests for remaining country metadata, admin audit, KV, and cache branches, then raise the coverage ratchet toward 83%. |
