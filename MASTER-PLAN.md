...
| Coverage depth | 8/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is about 89% lines and 73% branches. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage from 73% toward 80%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
