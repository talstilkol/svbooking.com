...
| #11 | `eslint` 9.39.4 -> ESLint 10 | BLOCKED - do not merge. Re-audited on July 26, 2026 against ESLint 10.8.0 and `eslint-config-next` 16.2.12: the config accepts ESLint `>=9`, but its current React, JSX accessibility, and import plugins still exclude ESLint 10 from their peer ranges. The earlier CI attempt also failed in `react/display-name`. Dependabot continues to ignore `eslint` semver-major updates. | Revisit only when the complete `eslint-config-next` plugin stack declares ESLint 10 support and passes lint without overrides |
...
| July Next patch alignment | `next`, `@next/bundle-analyzer`, and `eslint-config-next` -> 16.2.12 | DONE - runtime, analyzer, lint config, and `@next/eslint-plugin-next` resolve to the same Next patch. This does not remove the ESLint 10 plugin compatibility blocker. | Production audit reports 0 vulnerabilities; dependency tree, lint, typecheck, 200/1200 tests, 100% coverage, 729-page build, and 78 E2E tests passed locally |
