...
| Coverage depth | 8/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is 93.25% lines and 82.25% branches. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage from 82.25% toward 83%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
