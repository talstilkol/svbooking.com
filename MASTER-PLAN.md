...
| Coverage depth | 9/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is 99.44% lines and 99.72% branches. |
...
| Coverage ratchet current floor | DONE | `npm run audit:coverage` passes at 99.44% lines, 99.09% statements, 97.39% functions, 99.72% branches; floors are now lines 99.4%, statements 99.0%, functions 97.3%, branches 99.7%. |
...
| Stabilization Priority | Raise branch coverage from 84.07% toward 85% | DONE | Current branch coverage is 99.72% and the floor is 99.7%. |
...
| Acceptance Criteria | `npm run audit:coverage` passes | DONE | Passed at 99.44% lines and 99.72% branches. |
...
| Backlog P1 | Raise branch coverage from 84.07% toward 85% | DONE | Current branch coverage is 99.72%. |
| Backlog P1 | Raise branch coverage from 85.08% toward 88% | DONE | Current branch coverage is 99.72% and the floor is 99.7%. |
| Backlog P1 | Raise branch coverage from 88.14% toward 90% | DONE | Current branch coverage is 99.72% and the floor is 99.7%. |
| Backlog P1 | Raise branch coverage from 90.04% toward 92% | DONE | Current branch coverage is 99.72%; the ratchet floor is now 99.7%. |
...
| Backlog P1 | Raise `lib` branch coverage toward 99.3% | DONE | Coverage reached 99.72% branches and the ratchet floor was raised above this target. |
| Backlog P1 | Raise `lib` branch coverage toward 99.5% | DONE | Coverage reached 99.72% branches and the ratchet floor was raised above this target. |
| Backlog P1 | Raise `lib` branch coverage toward 99.6% | DONE | Coverage reached 99.72% branches and the ratchet floor was raised above this target. |
| Backlog P1 | Raise `lib` branch coverage toward 99.7% | DONE | Coverage reached 99.72% branches and the ratchet floor was raised to 99.7%. |
| Backlog P1 | Raise `lib` branch coverage toward 99.8% | PARTIAL | Remaining weak areas include private/inaccessible hotels-catalog, hotel-popularity, ops-alerts, webhook URL, and price-cache branches. |
...
| --- | --- | --- |
| Raise `lib` branch coverage from 83.26% toward 84%, then keep ratcheting upward. | DONE | Real and verified by the current 99.72% branch coverage result. |
...
| Raise `lib` branch coverage beyond 99.1%, focusing on provider observability and catalog media edge branches. | DONE | Real and verified at 99.72% branches. |
| Raise `lib` branch coverage toward 99.3%, focusing on remaining private/inaccessible hotels-catalog, price-cache, URL-safety, auth, and API/network error branches. | DONE | Real and verified at 99.72% branches. |
| Raise `lib` branch coverage toward 99.5%, focusing on remaining private/inaccessible hotels-catalog, webhook URL, auth, hotel-popularity, URL-safety, and API/network error branches. | DONE | Real and verified at 99.72% branches. |
| Raise `lib` branch coverage toward 99.6%, focusing on remaining private/inaccessible hotels-catalog, auth, hotel-popularity, Xotelo, ops-alerts, cheaper-dates, Wikivoyage, Ticketmaster, and price-cache branches. | DONE | Real and verified at 99.72% branches. |
| Raise `lib` branch coverage toward 99.7%, focusing on remaining private/inaccessible hotels-catalog, auth, hotel-popularity, agent-utils, Xotelo, ops-alerts, webhook URL, price-cache, and cheaper-dates branches. | DONE | Real and verified at 99.72% branches; next honest target is 99.8%. |
...
| Raise `lib` branch coverage toward 99.8%, focusing on remaining private/inaccessible hotels-catalog, hotel-popularity, ops-alerts, webhook URL, and price-cache branches. | PARTIAL | Next local engineering task after the current 99.72% branch coverage result. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage beyond 99%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
   - Prioritize remaining private/inaccessible hotels-catalog, provider observability, price-cache, URL-safety, auth, retention edge cases, and API/network error branches.
...
- [ ] Raise `lib` branch coverage toward 99.8%, focusing on remaining private/inaccessible hotels-catalog, hotel-popularity, ops-alerts, webhook URL, and price-cache branches.
