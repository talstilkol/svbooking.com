...
| Coverage depth | 6/10 | `npm run audit:coverage` now enforces a conservative ratchet floor; current `lib` coverage is about 61% lines and 53% branches. |
...
3. **Coverage ratchet**
   - Raise `lib` line coverage from 61% to 70%, then 80%, then raise the ratchet floors in `scripts/audit-coverage.mjs`.
