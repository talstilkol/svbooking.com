...
| Coverage depth | 8/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is 92.76% lines and 80.18% branches. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage from 80.18% toward 82%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
