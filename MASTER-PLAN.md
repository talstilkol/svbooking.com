...
| Coverage depth | 9/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is 98.97% lines and 94.17% branches. |
...
| Coverage ratchet current floor | DONE | `npm run audit:coverage` passes at 98.97% lines, 97.66% statements, 96.97% functions, 94.17% branches. |
| Coverage to world-class depth | PARTIAL | Branch coverage is above 94%, not near exhaustive; remaining weak areas include hotels-catalog, Overpass POI, Wikidata enrichment, health readiness, ops scorecard, Ticketmaster, alert, weather, and Wikipedia edges. |
...
| Current State | Coverage depth | PARTIAL | Ratchet passes at 94.17% branch coverage; next gap is 96%+ and exhaustive branch coverage in catalog, enrichment, ops, provider, weather, and event delivery. |
...
| Accountability | Coverage to world-class depth | PARTIAL | Branch coverage is above 94%, still not near exhaustive. |
...
| Stabilization Priority | Raise branch coverage from 84.07% toward 85% | DONE | Current branch coverage is 94.17% and the floor is 94.1%. |
| Stabilization Priority | Prioritize remaining branch coverage hot spots | PARTIAL | Overpass, provider registry, i18n, ops alert, price-cache, provider accuracy, public URL, geolocation, discovery, Wikidata, Google Places reviews, catalog candidate locking, provider uptime, OpenTripMap, Xotelo, rate-limit, admin audit, and ops scorecard branches were expanded; remaining hot spots include hotels-catalog, Overpass POI, Wikidata, health readiness, ops scorecard, Ticketmaster, alert, weather, and Wikipedia branches. |
...
| Acceptance Criteria | `npm test` passes | DONE | 166 files / 990 tests passed. |
...
| Acceptance Criteria | `npm run audit:coverage` passes | DONE | Passed at 98.97% lines and 94.17% branches. |
...
| Backlog P1 | Raise branch coverage from 84.07% toward 85% | DONE | Current branch coverage is 94.17%. |
| Backlog P1 | Raise branch coverage from 85.08% toward 88% | DONE | Current branch coverage is 94.17% and the floor is 94.1%. |
| Backlog P1 | Raise branch coverage from 88.14% toward 90% | DONE | Current branch coverage is 94.17% and the floor is 94.1%. |
| Backlog P1 | Raise branch coverage from 90.04% toward 92% | DONE | Current branch coverage is 94.17%; the ratchet floor is now 94.1%. |
| Backlog P1 | Add focused tests for remaining weak branches | PARTIAL | Focused network, provider, cache, i18n, ops, URL-safety, geolocation, discovery, Wikidata, Google Places reviews, catalog candidate locking, provider uptime, OpenTripMap, Xotelo, rate-limit, admin audit, ops scorecard, dynamic catalog, cheaper-date fallback, catalog candidates, KV adapter, webhook, and DBpedia/Wikivoyage tests were added; hotels-catalog, Overpass POI, Wikidata, health readiness, ops scorecard, Ticketmaster, alert, weather, and Wikipedia branches remain. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage from 94.17% toward 96%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
   - Prioritize hotels-catalog, Overpass POI, Wikidata enrichment, health readiness, ops scorecard, Ticketmaster, alert delivery, weather, Wikipedia, retention edge cases, and remaining API error branches.
...
- [ ] Raise `lib` branch coverage from 94.17% toward 96%, focusing on hotels-catalog, Overpass POI, Wikidata enrichment, health readiness, ops scorecard, Ticketmaster, alert delivery, weather, and Wikipedia branches.
...
| 3. Quality ratchet | Raise branch coverage to 94%, then 96%+, focusing on catalog, Overpass, Wikidata, health readiness, ops, provider, alert, weather, event, storage, and network failure paths. | Coverage floors are increased after each verified pass and CI blocks regression. |
