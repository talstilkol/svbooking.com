...
| Coverage depth | 8/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is about 88% lines and 72% branches. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage from 72% toward 80%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
   - Prioritize holidays, hotels-catalog, hotel-popularity, provider registry, cache, alert delivery, retention edge cases, and remaining API error branches.
