...
| Coverage depth | 8/10 | `npm run audit:coverage` now enforces a ratchet floor; current `lib` coverage is 91.67% lines and 77.74% branches. |
...
3. **Coverage ratchet**
   - Raise `lib` branch coverage from 77.74% toward 80%, then raise the ratchet floors again in `scripts/audit-coverage.mjs`.
