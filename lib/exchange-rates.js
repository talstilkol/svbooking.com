...
// Source: open.er-api.com (166 currencies, updates daily)
...
export async function getExchangeRates(base = 'USD') {
  ...
  if (rateCache.base === base && rateCache.rates && now - rateCache.fetchedAt < CACHE_TTL) {
    return { base: rateCache.base, rates: rateCache.rates, updatedAt: new Date(rateCache.fetchedAt).toISOString(), cached: true };
  }
  ...
  try {
    ...
    if (data?.rates) {
      ...
      return { base, rates: data.rates, updatedAt: new Date().toISOString(), cached: false };
    }
  } catch {
    // Try fallback
  }
  ...
  try {
    ...
    if (rates) {
      ...
      rateCache = { base, rates: normalized, fetchedAt: now };
      return { base, rates: normalized, updatedAt: new Date().toISOString(), cached: false };
    }
  } catch {
    // Both failed
  }
