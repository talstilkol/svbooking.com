 ...
 * When the same OTA appears from different providers, keep the lowest total
...
function mergeProviderRates(providerResults) {
  ...
  const mergedRates = Array.from(ratesByOTA.values()).map((rate) => {
    const cleaned = { ...rate };
    delete cleaned._total;
    return cleaned;
  });
...
class ProviderRegistry {
  ...
  async fetchRates(params) {
    ...
    // Filter the top available providers that can handle this request.
    const candidateProviders = available.slice(0, 3);
    const capable = candidateProviders.filter((p) => !p.canHandle || p.canHandle(params));
