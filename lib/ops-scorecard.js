...
import { buildCompetitorParity } from './competitor-parity';
import { buildCatalogMediaQuality } from './catalog-media-quality';
import { areLaunchServicesReady, buildLaunchServiceBlockers } from './launch-services.mjs';
...
function entry({ id, label, status, scoreValue, current, target, actionLedger = null, blockers = [], nextActions = [] }) {
  return {
    id,
    label,
    status,
...
export function buildOpsScorecard({
  env = process.env,
  now = new Date(),
  catalogMediaQuality = buildCatalogMediaQuality(),
} = {}) {
  const health = buildHealthSnapshot({ env, now, catalogMediaQuality });
  const checks = health.checks;
  const pwa = checks.pwa;
  const competitorParity = buildCompetitorParity({ checks, pwa, now });
  const catalogMediaReady = catalogMediaQuality.status === 'healthy';
  const launchServicesReady = areLaunchServicesReady(checks.launchServices);
  const coreProductionReady = Boolean(
    checks.catalog.ok &&
    checks.security.productionReady &&
    checks.security.adminSecretConfigured &&
    checks.security.cronSecretConfigured &&
  ...
  const freeOnlyLaunchReady = health.launchReadiness.freeOnlyLaunchReady;

  const domains = [
    entry({
      id: 'production-readiness',
      label: 'Production readiness',
      status: coreProductionReady ? 'healthy' : 'blocked',
      scoreValue: (
        score(checks.security.productionReady) +
        score(checks.security.adminSecretConfigured) +
        score(checks.security.cronSecretConfigured) +
        score(checks.security.kindeConfigured) +
        score(checks.cache.durable) +
        score(checks.providers.partnerConfigured) +
        score(catalogMediaReady) +
        score(launchServicesReady)
      ) / 8,
      current: {
        adminSecretConfigured: checks.security.adminSecretConfigured,
        cronSecretConfigured: checks.security.cronSecretConfigured,
        ...
        catalogMediaQuality: catalogMediaQuality.status,
        launchServicesReady,
      },
      target: {
        ...
        catalogMediaQuality: 'healthy',
        launchServicesReady: true,
      },
      blockers: [
        !checks.security.adminSecretConfigured && 'Configure ADMIN_API_SECRET',
        !checks.security.cronSecretConfigured && 'Configure CRON_SECRET',
        ...
        ...buildLaunchServiceBlockers(checks.launchServices),
      ].filter(Boolean),
    ...
    entry({
      id: 'catalog-media-quality',
      label: 'Catalog media quality',
      status: catalogMediaQuality.status,
      scoreValue: catalogMediaQuality.score,
      current: catalogMediaQuality.current,
      target: catalogMediaQuality.target,
      actionLedger: catalogMediaQuality.actionLedger || null,
    ...
    entry({
      ...
      status: pwa.push.configured && checks.alerts.deliveryConfigured && checks.alerts.unsubscribeConfigured ? 'healthy' : 'partial',
      scoreValue: (
        score(pwa.installable) +
        score(checks.alerts.deliveryConfigured) +
        score(checks.alerts.unsubscribeConfigured) +
        score(pwa.push.configured)
      ) / 4,
      current: {
        pwaStatus: pwa.status,
        alertDelivery: checks.alerts.deliveryStatus,
        unsubscribe: checks.alerts.unsubscribeStatus,
      ...
      blockers: [
        ...
        !checks.alerts.unsubscribeConfigured && 'Configure PRICE_ALERT_UNSUBSCRIBE_SECRET',
    ...
    entry({
      id: 'competitor-parity',
      label: 'Competitor parity',
      status: competitorParity.status,
      scoreValue: competitorParity.score,
      current: {
        score: competitorParity.score,
        sourcePolicy: competitorParity.sourcePolicy,
        lastReviewedAt: competitorParity.lastReviewedAt,
        trackedCompetitors: competitorParity.competitors.map((competitor) => competitor.id),
      },
      target: {
        inventoryBreadth: 'global-market-scale',
        priceFreshness: 'partner-provider-production-proof',
        mobileInstallability: 'installable-pwa-plus-push',
        reviews: 'licensed-provider-backed',
        alerts: 'delivery-and-unsubscribe-configured',
        bookingHandoffQuality: 'verified-provider-or-contracted-handoff',
      },
      blockers: competitorParity.blockers.map((item) => `${item.capability}: ${item.blocker}`),
      nextActions: competitorParity.nextActions,
    }),
  ];
