...
function cleanText(value, fallback = 'unknown') {
  const fallbackText = String(fallback).trim().slice(0, 80) || 'unknown';
...
function summarize(events, providerId) {
  ...
  return {
    ...
    successes,
    failures,
    successRatePct: Math.round((successes / providerEvents.length) * 1000) / 10,
    avgLatencyMs,
    p95LatencyMs: percentile(latencies, 95),
    lastCheckedAt: latest.checkedAt || null,
    lastStatus: latest.ok ? 'ok' : 'failed',
