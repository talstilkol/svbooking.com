...
const AMENITY_TAG_MAP = {
  'internet_access': { match: ['wlan', 'yes', 'wifi'], label: 'Free WiFi', icon: '📶' },
...
export async function getHotelAmenities({ lat, lon, hotelName, timeoutMs = 15000 }) {
  if (!lat || !lon || !hotelName) return null;

  // Escape special regex chars in hotel name, use case-insensitive match
  const escaped = hotelName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  // Use a shorter version of the name for matching (first 2+ words)
  const shortName = escaped.split(/\s+/).slice(0, 3).join(' ');
  ...
  return data.elements
    .map((el) => {
      ...
      return {
        ...
        type: typeInfo.type,
        icon: typeInfo.icon,
        distance: formatDistance(haversine(centerLat, centerLon, elLat, elLon)),
        distanceM: haversine(centerLat, centerLon, elLat, elLon),
        lat: elLat,
        lon: elLon,
...
function extractAmenities(tags) {
  ...
  // Also check combined tags
  if (tags['internet_access:fee'] === 'no') {
    if (!amenities.find((a) => a.label === 'Free WiFi')) {
