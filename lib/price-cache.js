...
const BLOCKED_PROVIDER_VALUES = new Set(['none', 'unknown', 'unavailable']);
...
function adaptiveTTLs(checkIn) {
  ...
  const tier = TTL_TIERS.find((t) => daysOut <= t.maxDays);
  return { freshTTL: tier.freshTTL, staleTTL: tier.staleTTL };
}

/** Add deterministic +/-15% jitter to a TTL value to prevent thundering herd */
export function jitteredTTL(baseTTL, seed) {
  if (!seed) throw new Error('TTL jitter seed is required');
  const jitter = baseTTL * 0.15;
  const hash = hashId('ttl-jitter', seed, baseTTL).slice(2);
  const unit = parseInt(hash, 36) / 0xffffffff;
  const offset = (unit * 2 - 1) * jitter;
...
function ignoreBackgroundCacheError() {}
...
function cleanProviderText(value) {
  ...
  if (!text) return null;
  return BLOCKED_PROVIDER_VALUES.has(text.toLowerCase()) ? null : text;
}

function normalizeCurrency(value, fallback = 'USD') {
  const currency = cleanText(value)?.toUpperCase();
  if (currency && CURRENCY_PATTERN.test(currency)) return currency;
  const fallbackCurrency = cleanText(fallback)?.toUpperCase();
  return fallbackCurrency && CURRENCY_PATTERN.test(fallbackCurrency) ? fallbackCurrency : 'USD';
}
...
function normalizeRate(rate, result, { checkedAt, fromCache, freshness, partial }) {
  ...
  const providerClaim = cleanText(rate?.provider) || cleanText(rate?.name);
  const provider = cleanProviderText(rate?.provider)
    || cleanProviderText(rate?.name)
    || (providerClaim ? null : cleanProviderText(result?.provider))
    || (providerClaim ? null : cleanProviderText(result?.source));
  if (!provider) return null;

  const source = cleanProviderText(rate?.source)
    || cleanProviderText(result?.source)
    || cleanProviderText(result?.provider)
    || provider;
  const currency = normalizeCurrency(rate?.currency, result?.currency || 'USD');
  ...
  return {
    ...
    cancellationPolicy: cleanText(rate?.cancellationPolicy),
    roomName: cleanText(rate?.roomName),
    url: normalizeHttpsUrl(rate?.url),
    deepLink: normalizeHttpsUrl(rate?.deepLink || rate?.url),
...
function normalizeRatesResult(result, { fromCache, freshness, partial, checkedAt = nowIso() }) {
  const rates = Array.isArray(result?.rates)
    ? result.rates
      .map((rate) => normalizeRate(rate, result, { checkedAt, fromCache, freshness, partial }))
      .filter(Boolean)
    : [];
  const provider = cleanProviderText(result?.provider) || cleanProviderText(result?.source) || 'none';
  const source = cleanProviderText(result?.source) || cleanProviderText(result?.provider) || provider;
...
async function fetchAndCacheRates(key, params, { staleTTL } = {}) {
  ...
  const effectiveStaleTTL = staleTTL;
  ...
  const promise = (async () => {
    ...
    kv.setWithTTL(key, { result: normalized, cachedAt: checkedAt }, jitteredTTL(effectiveStaleTTL, key)).catch(() => {});
    ...
    if (params.hotelKey && normalized.rates?.length > 0) {
      ...
      }, jitteredTTL(effectiveStaleTTL * 2, latestKey)).catch(() => {});
    }
...
export async function getCachedRates({ hotelKey, hotelName, city, checkIn, checkOut, currency = 'USD', timeoutMs }) {
  ...
  try {
    ...
    if (envelope) {
      ...
      if (!fresh) {
        fetchAndCacheRates(key, { hotelKey, hotelName, city, checkIn, checkOut, currency, timeoutMs }, { staleTTL }).catch(ignoreBackgroundCacheError);
      }
  ...
  // Live fetch — if this fails, attempt stale-if-error fallback
  try {
    return await fetchAndCacheRates(key, { hotelKey, hotelName, city, checkIn, checkOut, currency, timeoutMs }, { staleTTL });
  } catch (fetchErr) {
    // Stale-if-error: when all providers fail, try to serve any cached data
    // rather than returning an error. Check fuzzy cache one more time.
    try {
      const latestKey = `latest-rates:${hotelKey}:${currency}`;
      const fallback = await kv.get(latestKey);
      if (fallback?.result?.rates?.length > 0) {
        const normalized = normalizeRatesResult(fallback.result, {
          fromCache: true,
          freshness: 'fallback',
          partial: true,
          checkedAt: fallback.result?.lastCheckedAt || fallback.cachedAt,
        });
        if (fallback.forDates) normalized.estimatedFromDates = fallback.forDates;
        return normalized;
      }
    } catch {
...
export async function getCachedHeatmap({ hotelKey, checkOut, timeoutMs }) {
  ...
  kv.setWithTTL(key, normalized, jitteredTTL(HEATMAP_TTL, key)).catch(ignoreBackgroundCacheError);

  // Seed the fuzzy date cache from heatmap data.
  // Heatmaps return per-date rates; use the lowest returned value to populate
  // `latest-rates` so fuzzy date matching has data without extra API calls.
  if (result?.rates?.length > 0 && hotelKey) {
    try {
      // Find the lowest returned rate from the heatmap.
      const cheapest = result.rates.reduce((min, r) =>
        (r.rate > 0 && (min === null || r.rate < min.rate)) ? r : min, null);
      if (cheapest) {
        const latestKey = `latest-rates:${hotelKey}:USD`;
        const seedResult = {
          rates: [{ name: 'Xotelo (heatmap)', rate: cheapest.rate, tax: 0 }],
          currency: 'USD',
          provider: 'Xotelo',
          source: 'xotelo',
          lastCheckedAt: nowIso(),
        };
        // Only seed if no existing latest-rates (don't overwrite richer live data)
        kv.get(latestKey).then((existing) => {
          if (!existing) {
            kv.setWithTTL(latestKey, {
              result: seedResult,
              cachedAt: nowIso(),
              forDates: { checkIn: cheapest.date || checkOut, checkOut },
            }, jitteredTTL(HEATMAP_TTL, latestKey)).catch(ignoreBackgroundCacheError);
          }
        }).catch(ignoreBackgroundCacheError);
      }
    } catch {
      // Non-critical — fuzzy seeding is best-effort
    }
  }
...
export async function getCachedRatesBatch(paramsList) {
  ...
  return Promise.all(paramsList.map(async (params, i) => {
    ...
    if (envelope) {
      ...
      if (!fresh) {
        fetchAndCacheRates(key, params, { staleTTL }).catch(ignoreBackgroundCacheError);
      }
      return normalized;
    }
