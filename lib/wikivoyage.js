...
async function getSections(city, timeoutMs = DEFAULT_TIMEOUT) {
  const url = `${WIKI_VOYAGE_MW}?action=parse&page=${encodeURIComponent(city)}&prop=sections&format=json&origin=*`;
...
async function getSectionContent(city, sectionIndex, timeoutMs = DEFAULT_TIMEOUT) {
  const url = `${WIKI_VOYAGE_MW}?action=parse&page=${encodeURIComponent(city)}&section=${sectionIndex}&prop=text&format=json&origin=*`;
 ...
 *
 * @param {string} city
 * @returns {Promise<{overallRating: number, tips: string[], areas: Array<{name: string, safe: boolean, note: string}>, vaccinations: string, waterSafety: string} | null>}
 */
 ...
 *
 * @param {string} city
 * @returns {Promise<Array<{name: string, month: string, icon: string, description: string}> | null>}
 */
export async function getEventInfo(city, timeoutMs = DEFAULT_TIMEOUT) {
  ...
  return parseEventInfo(html, understandHtml, city);
}
 ...
 *
 * @param {string} city
 * @returns {Promise<Array<{name: string, description: string, type: string}> | null>}
 */
...
function parseSafetyInfo(safetyHtml, healthHtml) {
  const tips = [];
  const areas = [];
  let vaccinations = 'None required';
  let waterSafety = 'Check local advice';
  ...
  // Calculate overall rating (1-5) based on text sentiment
  let overallRating = 3; // Default neutral
  const allText = (stripHtml(safetyHtml || '') + ' ' + stripHtml(healthHtml || '')).toLowerCase();
  if (/very safe|extremely safe|one of the safest|perfectly safe/.test(allText)) overallRating = 5;
  else if (/generally safe|quite safe|relatively safe|safe for tourists/.test(allText)) overallRating = 4;
  else if (/exercise caution|be careful|some risk|petty crime|pickpocket/.test(allText)) overallRating = 3;
  else if (/dangerous|unsafe|avoid|violent crime|high crime/.test(allText)) overallRating = 2;

  return {
    overallRating,
    tips: tips.length > 0 ? tips : ['Check local travel advisories for current safety information.'],
...
function parseEventInfo(doHtml, understandHtml, city) {
  const events = [];
  ...
  while ((match = boldPattern.exec(allHtml)) !== null) {
    ...
    const description = descText.length > 20 ? descText.slice(0, 120).trim() + '...' : `Annual event in ${city}`;
...
function parseDiningInfo(html) {
  ...
  while ((match = boldPattern.exec(html)) !== null) {
    ...
    items.push({
      name,
      description: description || 'Local dining spot',
      type: 'restaurant',
    });
  }
