...
export function buildProductionReadinessSummary({ env = process.env, strict = env.PRODUCTION_READINESS_STRICT === '1' } = {}) {
  ...
  return {
    productionReady: missingRequired.length === 0 && missingKinde.length === 0 && configuredPricing.length > 0,
    strict,
    required,
    kinde,
    pricingProviders,
    ...
    notes: [
      ...
      'Xotelo can still serve as a no-auth baseline provider, but production scale requires durable KV, Kinde auth, and at least one complete partner pricing provider env group.',
    ],
  };
}
