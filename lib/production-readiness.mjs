...
function hasValidWebhookConfig(env, urlName, secretName) {
  return Boolean(
...
function isGoogleReviewProvider(providerName) {
  const normalized = String(providerName || '').trim().toLowerCase();
  return normalized === 'google' || normalized === 'google-places';
}

function summarizeLaunchServices(env) {
  ...
  const supportedReviewProvider = isGoogleReviewProvider(reviewProviderName);
...
export function buildProductionReadinessSummary({
  ...
  const launchServicesReady = launchServices.reviews.configured &&
