...
function providerAlerts(uptime) {
  ...
  for (const provider of uptime.providers || []) {
    if (provider.successRatePct !== null && provider.successRatePct < ALERT_THRESHOLDS.providerCriticalSuccessRatePct) {
      alerts.push(alert({
        id: `provider-${provider.providerId}-critical-success-rate`,
        severity: 'critical',
        domain: 'provider-uptime',
        message: `${provider.providerName} success rate is below critical threshold.`,
        evidence: {
          providerId: provider.providerId,
          successRatePct: provider.successRatePct,
          thresholdPct: ALERT_THRESHOLDS.providerCriticalSuccessRatePct,
          eventCount: provider.total,
        },
      ...
      alerts.push(alert({
        id: `provider-${provider.providerId}-warning-success-rate`,
        severity: 'warning',
        domain: 'provider-uptime',
        message: `${provider.providerName} success rate is below target.`,
        evidence: {
          providerId: provider.providerId,
          successRatePct: provider.successRatePct,
    ...
    if (provider.p95LatencyMs !== null && provider.p95LatencyMs > ALERT_THRESHOLDS.providerCriticalP95LatencyMs) {
      alerts.push(alert({
        id: `provider-${provider.providerId}-critical-latency`,
        severity: 'critical',
        domain: 'provider-latency',
        message: `${provider.providerName} p95 latency is above critical threshold.`,
        evidence: {
          providerId: provider.providerId,
      ...
      alerts.push(alert({
        id: `provider-${provider.providerId}-warning-latency`,
        severity: 'warning',
        domain: 'provider-latency',
        message: `${provider.providerName} p95 latency is above target.`,
        evidence: {
          providerId: provider.providerId,
          p95LatencyMs: provider.p95LatencyMs,
