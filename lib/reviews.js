...
export async function getReviewSummary(hotelKey, { env = process.env, fetchImpl } = {}) {
  ...
  const providerName = String(env.REVIEWS_PROVIDER_NAME || '').toLowerCase();

  try {
    if (providerName === 'google-places' || providerName === 'google') {
    ...
    // Unknown provider name or empty data → fall through to unavailable.
  } catch (err) {
