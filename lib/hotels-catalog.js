...
function parseTripAdvisorHotelKey(value) {
  const match = String(value || '').match(/^g(\d+)-d(\d+)$/);
  return match ? {
    tripadvisorLocationId: match[1],
    tripadvisorHotelId: match[2],
  } : null;
}

export function getStaticCatalogItemProvenance(hotel) {
  const externalIds = parseTripAdvisorHotelKey(hotel?.hotelKey);

  return {
    status: externalIds ? 'source-metadata-available' : 'missing-source-key',
    ...STATIC_CATALOG_SOURCE_POLICY,
    sourceUrl: null,
    externalIds: externalIds || {},
  };
}

export function getStaticCatalogImageProvenance(hotel) {
  const sourceUrl = typeof hotel?.image === 'string' ? hotel.image.trim() : '';
  if (!sourceUrl) {
    return {
      ...
      approvedLicense: false,
      replacementRequired: true,
      dataPolicy: 'missing-image-source',
    };
  }
  ...
  try {
    ...
    return {
      status: 'invalid-image-source-url',
      source: null,
      sourceHost: null,
      sourceUrl,
      licenseStatus: 'missing',
      approvedLicense: false,
      replacementRequired: true,
      dataPolicy: 'invalid-image-source',
    };
  }

  const policy = STATIC_IMAGE_SOURCE_POLICIES.get(parsed.hostname);
  return {
    status: policy ? 'source-metadata-available' : 'source-host-not-allowlisted',
    source: policy?.source || null,
    sourceHost: parsed.hostname,
    sourceUrl: parsed.toString(),
    licenseStatus: policy?.licenseStatus || 'missing',
    approvedLicense: policy?.approvedLicense === true,
    replacementRequired: policy?.replacementRequired !== false,
    dataPolicy: policy?.dataPolicy || 'image-source-url-only',
  };
}

export function buildStaticCatalogProvenanceLedger({ hotels = HOTELS } = {}) {
  const catalogHotels = Array.isArray(hotels) ? hotels : [];
  return catalogHotels.map((hotel) => ({
    hotelKey: typeof hotel?.hotelKey === 'string' ? hotel.hotelKey : null,
    name: typeof hotel?.name === 'string' ? hotel.name : null,
    city: typeof hotel?.city === 'string' ? hotel.city : null,
    country: typeof hotel?.country === 'string' ? hotel.country : null,
    catalogItem: getStaticCatalogItemProvenance(hotel),
    image: getStaticCatalogImageProvenance(hotel),
  }));
}
