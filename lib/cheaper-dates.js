...
/**
 * Fetch heatmaps in parallel batches (concurrency limit).
 * Each heatmap call for a given checkOut returns an array of { date, price } for multiple check-in dates.
 */
async function fetchHeatmapBatch(hotelKey, checkoutDates, concurrency = 5) {
  ...
  for (let i = 0; i < checkoutDates.length; i += concurrency) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (checkOut) => {
        const data = await getHeatmap({ hotelKey, checkOut });
...
export async function findCheaperDates(hotelKey, checkIn, checkOut) {
  ...
  // 4. Fetch heatmaps in batch (5 concurrent)
  const heatmapResults = await fetchHeatmapBatch(hotelKey, checkoutsToFetch, 5);
    ...
    for (let i = 0; i < fallbackTasks.length; i += 5) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (task) => {
          const result = await getRates({ hotelKey, checkIn: task.checkIn, checkOut: task.checkOut });
          const rates = (result?.rates || [])
