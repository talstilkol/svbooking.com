  ...
  for (let i = 0; i < checkoutDates.length; i += concurrency) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (checkOut) => {
        const data = await getHeatmap({ hotelKey, checkOut, timeoutMs: BATCH_TIMEOUT_MS });
...
export async function findCheaperDates(hotelKey, checkIn, checkOut) {
  ...
  try {
    const result = await getRates({ hotelKey, checkIn, checkOut, timeoutMs: BATCH_TIMEOUT_MS });
    ...
    for (let i = 0; i < fallbackTasks.length; i += 5) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (task) => {
          const result = await getRates({ hotelKey, checkIn: task.checkIn, checkOut: task.checkOut, timeoutMs: BATCH_TIMEOUT_MS });
          const rates = (result?.rates || [])
