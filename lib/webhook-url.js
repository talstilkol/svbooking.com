...
const DOCUMENTATION_WEBHOOK_HOSTS = new Set(['example.com', 'example.org', 'example.net']);
const IPV4_DECIMAL_OCTET = '(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)';
const IPV4_DECIMAL_PATTERN = new RegExp(`^${IPV4_DECIMAL_OCTET}(?:\\.${IPV4_DECIMAL_OCTET}){3}$`);
...
function parseIpv4Octets(hostname) {
  const normalized = normalizeWebhookHostname(hostname);
  if (!IPV4_DECIMAL_PATTERN.test(normalized)) return null;
...
function isRestrictedWebhookHost(hostname) {
  const normalized = normalizeWebhookHostname(hostname);
  const documentationHost = Array.from(DOCUMENTATION_WEBHOOK_HOSTS).some(
    (host) => normalized === host || normalized.endsWith(`.${host}`)
  );
  return (
    isLocalWebhookHost(normalized) ||
    isPrivateIpv4(normalized) ||
    isRestrictedIpv6(normalized) ||
    documentationHost ||
