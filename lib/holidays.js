...
// Returns official public holidays for 100+ countries.
...
async function fetchWithTimeout(url, timeoutMs = 8000) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (!res.ok) throw new Error(`Holidays API HTTP ${res.status}`);
    return await res.json();
  } catch (err) {
    clearTimeout(timer);
    if (err.name === 'AbortError') throw new Error(`Holidays request timed out`);
    throw err;
  }
}
