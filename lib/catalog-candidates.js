...
function withValidationFlags(candidate) {
  ...
  const duplicate = Boolean(normalizedCandidate.alreadyInCatalog || normalizedCandidate.duplicate);
  const flags = [];
  ...
  if (duplicate && !normalizedCandidate.alreadyInCatalog) flags.push('duplicate-fingerprint');
...
function sortedCountBuckets(counts, limit = REVIEW_SUMMARY_LIMIT) {
...
function addBucketCount(counts, value) {
  const normalizedValue = normalizeNullableText(value) || 'unavailable';
  counts.set(normalizedValue, (counts.get(normalizedValue) || 0) + 1);
}

function hasPromotionFields(candidate) {
  return Boolean(candidate?.hotelKey && candidate?.name && candidate?.city && candidate?.country);
}

function isReadyToApprove(candidate) {
  return candidate?.status === 'pending' &&
    !candidate.duplicate &&
    !candidate.missingProvenance &&
    !candidate.missingLocation &&
    hasPromotionFields(candidate);
}

export function buildCandidateReviewSummary(candidates = []) {
  const normalizedCandidates = (Array.isArray(candidates) ? candidates : [])
    .filter(Boolean)
    .map((candidate) => withValidationFlags(candidate));
  const statusCounts = new Map();
  const sourceCounts = new Map();
  const cityCounts = new Map();
  const validationFlagCounts = new Map();
  ...
  for (const candidate of normalizedCandidates) {
    addBucketCount(statusCounts, candidate.status || 'unknown');
    addBucketCount(sourceCounts, candidate.source || candidate.provenance?.source || 'unavailable');
    addBucketCount(cityCounts, candidate.city || candidate.discoveredForCity || 'unavailable');

    for (const flag of candidate.validationFlags) {
      addBucketCount(validationFlagCounts, flag);
    }

    if (isReadyToApprove(candidate)) {
      readyToApprove++;
    } else if (candidate.status === 'pending') {
      blockedPending++;
    }

    if (candidate.status === 'pending' && candidate.createdAt) {
      oldestPendingAt = oldestPendingAt === null || candidate.createdAt < oldestPendingAt
        ? candidate.createdAt
        : oldestPendingAt;
    }
    if (candidate.updatedAt) {
      newestUpdatedAt = newestUpdatedAt === null || candidate.updatedAt > newestUpdatedAt
        ? candidate.updatedAt
        : newestUpdatedAt;
    }
  }
  ...
  const missingPromotionFields = normalizedCandidates
    .filter((candidate) => !hasPromotionFields(candidate))
    .length;
  const duplicate = normalizedCandidates.filter((candidate) => candidate.duplicate).length;
  const missingProvenance = normalizedCandidates.filter((candidate) => candidate.missingProvenance).length;
  const missingLocation = normalizedCandidates.filter((candidate) => candidate.missingLocation).length;
  ...
  return {
    ...
    newHotels: normalizedCandidates.filter((candidate) => !candidate.alreadyInCatalog).length,
    existingHotels: normalizedCandidates.filter((candidate) => candidate.alreadyInCatalog).length,
    readyToApprove,
    promotable: readyToApprove,
    blockedPending,
    issueCount: duplicate + missingProvenance + missingLocation + missingPromotionFields,
    byStatus: sortedCountBuckets(statusCounts),
    bySource: sortedCountBuckets(sourceCounts),
    byCity: sortedCountBuckets(cityCounts),
    validationFlags: sortedCountBuckets(validationFlagCounts),
    reviewQueues: {
      readyToApprove,
      duplicate,
      missingProvenance,
