...
function hasUsableProvenance(candidate) {
  ...
  return Boolean(
    normalizeNullableText(candidate?.sourceUrl) ||
    normalizeNullableText(provenance.sourceUrl || provenance.url) ||
...
function withValidationFlags(candidate) {
  const flags = [];
  if (!hasUsableProvenance(candidate)) flags.push('missing-provenance');
  if (!hasVerifiedLocation(candidate)) flags.push('missing-location');
  if (candidate.alreadyInCatalog) flags.push('already-in-catalog');
  ...
  return {
    ...candidate,
    duplicate: Boolean(candidate.alreadyInCatalog),
    missingProvenance: !hasUsableProvenance(candidate),
    missingLocation: !hasVerifiedLocation(candidate),
    validationFlags: [...new Set([...(candidate.validationFlags || []), ...flags])],
  };
}
...
export function normalizeCandidate(input, { status = 'pending', source, discoveredForCity } = {}) {
  ...
  const sourceUrl = normalizeNullableText(input?.sourceUrl || input?.url || input?.provenance?.sourceUrl || input?.provenance?.url);
  ...
  const candidate = {
    ...
    provenance: input?.provenance || {
      source: resolvedSource,
      sourceUrl,
      wikidataId: externalIds.wikidataId || null,
      osmId: externalIds.osmId || null,
