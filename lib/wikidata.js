...
export async function discoverHotels({ country, city, limit = 200 } = {}) {
  ...
  let filters = '';
  if (country) {
    filters += `?hotel wdt:P17 ?country . ?country rdfs:label "${country}"@en .\n`;
  }
  if (city) {
    filters += `FILTER(CONTAINS(LCASE(?adminAreaLabel), LCASE("${city.replace(/"/g, '')}")))`;
  }
...
export async function getCityGeoIds(cityNames) {
  const values = cityNames.map((c) => `"${c}"@en`).join(' ');
  const query = `
