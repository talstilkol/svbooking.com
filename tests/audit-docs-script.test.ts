...
const packageJson = JSON.stringify({
  scripts: {
    ...
    'launch:readiness:report': 'node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON scripts/launch-readiness-report.mjs',
    'audit:launch-readiness-report': 'node scripts/audit-launch-readiness-report.mjs',
    'typecheck:debt:report': 'node scripts/typecheck-debt-report.mjs',
    'catalog:media:ledger': 'node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON scripts/catalog-media-action-ledger.mjs',
    'catalog:media:ledger:summary': 'node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON scripts/catalog-media-action-ledger.mjs --format=summary --priority-only',
    'catalog:media:ledger:csv': 'node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON scripts/catalog-media-action-ledger.mjs --format=csv',
    'catalog:media:ledger:priority-csv': 'node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON scripts/catalog-media-action-ledger.mjs --format=csv --priority-only',
...
const ci = [
  ...
  '  - run: npm run audit:launch-readiness-report',
...
const validReadme = [
  ...
  'Run npm run launch:readiness:report before launch review.',
  'Run npm run audit:launch-readiness-report before launch review.',
  'Run npm run typecheck:debt:report before typecheck cleanup.',
  'Run npm run catalog:media:ledger before launch.',
  'Run npm run catalog:media:ledger:summary before media review.',
  'Run npm run catalog:media:ledger:csv before media review.',
  'Run npm run catalog:media:ledger:priority-csv before priority media review.',
  'Use docs/LAUNCH-READINESS-CHECKLIST.md for launch readiness.',
...
const validAuditReport = [
  '# Audit Report',
  'Current local catalog: 1 hotels, 1 cities, 1 countries.',
  '200 test files, 1200 tests passed.',
...
const validRunbook = [
  ...
  'Run npm run launch:readiness:report before the full gate.',
  'Run npm run catalog:media:ledger:summary before media review.',
  'It is not a replacement for `audit:production:strict`.',
  '',
].join('\n');

const validLaunchChecklist = [
  '# SV Booking Launch Readiness Checklist',
  'Current baseline: 1 hotels, 1 cities, 1 countries.',
  'Current production audit reports productionReady: false.',
  'Run npm run catalog:media:ledger:summary before reviewing reused media.',
  'Run npm run catalog:media:ledger:csv for tabular media review.',
  'Run npm run catalog:media:ledger:priority-csv for priority tabular media review.',
...
const validFiles = {
  ...
  'PRODUCTION-RUNBOOK.md': validRunbook,
  'docs/LAUNCH-READINESS-CHECKLIST.md': validLaunchChecklist,
