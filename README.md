...
Run `npm run catalog:media:ledger` to print the exact catalog media replacement or license-approval queue without approving any image automatically. Use `npm run catalog:media:ledger:summary` and `npm run catalog:media:ledger:priority-csv` for the reused-media priority queue, or `npm run catalog:media:ledger:csv` for full operational review.
...
- Public data APIs must preserve source/dataPolicy/unavailable contracts so missing upstream data cannot look like confirmed evidence.
