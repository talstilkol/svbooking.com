...
Run every `npm run audit:*` script before release. `npm audit` contacts the npm registry and sends dependency metadata; run it only in an environment where that disclosure is approved.
